...
var findWithQuery = function(db, q_obj, callback) {
...
app.get('/fetchBoards', function(req,res) {
  ...
  mc.connect(url, function(err,db) {
    ...
    findWithQuery(db , 
        { "Geo"  : {$geoWithin: {$centerSphere: [[user_lat,user_long],  1.0/3963.2  ]}}},
        function (array) {
          res.json(array);
        })
  });
...
app.get('/fetchBoard', function(req,res) {
  ...
  mc.connect(url, function(err,db) {
    ...
    console.log("Connected correctly to server.");
    console.log("user_lat: "+user_lat+" user_long: " + user_long);
    findWithQuery(db, {"_id": "ObjectId("+board_id+")"} , function (array) {
      res.json(array);
    })
  });
});
